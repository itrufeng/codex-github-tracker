...
2. 以本 `SKILL.md` 为基准解析 `../_feature-workflow/github_feature.py`，然后执行 `create --summary <摘要> --request-file <文件>`。脚本会确定性地识别当前 GitHub repository 和 Project。对于新请求，它会创建 Issue、加入 Project、显式移至 Ready，再移至 In progress，同时在 Git local state 中保存 Issue number 和 Ready 步骤完成标记；不要假设新 Project item 具有任何初始 Status。如果已有 pending state，脚本只会在验证 repository、Project、Issue URL 和完整 Issue body 都与本次请求一致后，按已保存的 Issue number 恢复并补齐尚未完成的 Status 步骤；绝不能根据摘要搜索。读取标准输出的 JSON：
   - `created`：已创建新 Issue。
   - `resumed`：已恢复现有 Issue。
