...
三个技能默认静默运行，只报告议题编号、项目状态和最终结果。任何异常或中断都会立即停止，不会自动分析原因、重试或继续目标。
...
脚本把议题加入项目时，会要求 `gh project item-add` 返回 JSON，保存其中的项目项标识符，并直接使用该标识符将项目项从 Ready 移至 In progress。它不会立即依赖 `gh project item-list` 查到刚加入的项目项。对于没有项目项标识符的旧待处理状态，脚本会查找现有项目项，或以幂等方式重新添加。

如果恢复或手动加入的项目项最初落在 Backlog 等其他状态，恢复时会直接移动到目标列。状态转换只校验目标是否存在，并且可以安全重试。
...
项目状态修改和议题开关操作都是幂等的。`feature-done` 会先把项目项移到 Done，再以完成原因为议题执行关闭操作；重复执行时，已经完成的步骤不会重复修改。后续再次调用 `feature-continue` 时会重新打开议题并移到 In progress。
