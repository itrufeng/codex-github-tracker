...
脚本把议题加入项目时，会要求 `gh project item-add` 返回 JSON，保存其中的项目项标识符，然后显式将项目项移至 Ready，再移至 In progress。它不假设 GitHub 会赋予任何初始状态，也不会立即依赖 `gh project item-list` 查到刚加入的项目项。Ready 步骤完成后会保存标记，确保中断恢复时补齐步骤且不会无故回退；旧版 `backlog_set` 标记会迁移为 `ready_set`。对于没有项目项标识符的旧待处理状态，脚本会查找现有项目项，或以幂等方式重新添加。
